...
def octant_trilateration(signals, widths, mic_coordinates):
    # print 'Widths:', widths
    x_axis_indicies = [1, 0, 3, 2]
    y_axis_indicies = [3, 2, 1, 0]
    """
...
def localize_spark_pp(times, signals, v_s, v_p, grid_size, live):
  dt = times[1] - times[0]
  mic_coordinates = numpy.array(zip([5, -5, -5, 5], [6, 6, -6, -6]))
  radius = 14.22  # cm
  thickness = 1.37  # cm
